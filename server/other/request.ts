...
function convertToUrlEncoded(data: any): string {
    return getURLEncodedPayload(data)
        .toString();
}

// @ts-ignore
function getURLEncodedPayload(obj, searchParams = new URLSearchParams()) {
    Object
        .entries(obj)
        .forEach(([key, value]) => {
            if (value !== null && typeof value === 'object') {
                if (Array.isArray(value)) {
                    value.forEach((item, index) => {
                        getURLEncodedPayload({ [`${key}[${index}]`]: item }, searchParams);
                    });
                } else {
                    Object.entries(value).forEach(([subKey, subValue]) => {
                        getURLEncodedPayload({ [`${key}[${subKey}]`]: subValue }, searchParams);
                    });
                }
            } else {
                // @ts-ignore
